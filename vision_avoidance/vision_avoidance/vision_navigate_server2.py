...
class VisionNavigateServer(Node):
    ...
    def learn_floor(self, bgr):
        ...
        # k = 1.5
        # lo = np.array([mean[0] - k*std[0], max(mean[1] - k*std[1], 30), max(mean[2] - k*std[2], 30)])
        # hi = np.array([mean[0] + k*std[0], min(mean[1] + k*std[1], 255), min(mean[2] + k*std[2], 255)])
        # lo = np.clip(lo, [0,0,0], [179,255,255])
        # hi = np.clip(hi, [0,0,0], [179,255,255])
        self.floor_hsv = (lo.astype(np.uint8), hi.astype(np.uint8))


    # def inrange_hue_wrap(self, hsv, lo, hi):
    #     # If Hue range doesn’t cross 0, normal path
    #     if lo[0] <= hi[0]:
    #         return cv2.inRange(hsv, lo, hi)
    #     # If Hue wraps (e.g., 170..179 and 0..10), split in two
    ...
    def obstacle_mask(self, bgr):
        ...
        lo, hi = self.floor_hsv # Range of color that is the floor
        floor = cv2.inRange(hsv, lo, hi) # Creates binary mask where the white pixels are the floor
        # floor = self.inrange_hue_wrap(hsv, lo, hi)
        # core = self.inrange_hue_wrap(hsv, lo, hi)
        # S = hsv[:,:,1]; V = hsv[:,:,2]
        # spec_ok = cv2.inRange(hsv, (0,0,180), (179,50,255))  # low-S, high-V
        # floor = cv2.bitwise_or(core, spec_ok)
        
        obs = cv2.bitwise_not(floor) # Reserve it
        
        h, w = obs.shape
        roi_y = int(h * self.get_parameter('roi_y_ratio').value) 
        roi = obs[roi_y:h, :] # Just looking at  bottom 40% of img
        
        # Cleaning up noise
        kernel = np.ones((5,5), np.uint8)
        roi = cv2.morphologyEx(roi, cv2.MORPH_OPEN, kernel, iterations=1) # removes tiny white specks
        roi = cv2.morphologyEx(roi, cv2.MORPH_CLOSE, kernel, iterations=1) # fills tiny holes inside obstacle blobs (Makes white areas more clean)
        
        col_sum = roi.sum(axis=0).astype(np.float32) # Histogram
        col_sum /= 255.0 * (h - roi_y) # Normalized 
        free = 1.0 - col_sum # Free space 
        
        # Picking best coloumn to drive towards
        # w_window = max(7, int(w/30))
        # free_smooth = cv2.blur(free, (w_window, 1)).flatten()
        
        # w_window = max(7, (w // 30) | 1)  # force odd
        # free_smooth = cv2.blur(free.reshape(1,-1), (w_window,1)).flatten()
        w_window = max(7, (w // 30) | 1)                        # odd >=7
        free_smooth = cv2.blur(free.reshape(1, -1), (w_window, 1)).reshape(-1)
        best_col = int(np.argmax(free_smooth))
        # steer_norm = (best_col / (w-1))*2.0 - 1.0
        steer_norm = (best_col / float(max(1, w - 1))) * 2.0 - 1.0
        
        # Emergency band for if the very front is blocked
        band_y = int(h * self.get_parameter('front_band_ratio').value)
        band = obs[band_y:h, :]
        front_fill = band.mean() / 255.0
        band_y = int(h * self.get_parameter('front_band_ratio').value)
        band = obs[band_y:h, :]
        # focus on center 60% of the width
        cx0, cx1 = int(0.2*w), int(0.8*w)
        band_c = band[:, cx0:cx1].copy()

        # bigger cleanup to eat plank seams & pinholes
        k = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (9,9))
        band_c = cv2.morphologyEx(band_c, cv2.MORPH_CLOSE, k, iterations=2)
        band_c = cv2.morphologyEx(band_c, cv2.MORPH_OPEN,  k, iterations=1)

        # front_fill = band_c.mean()/255.0
        # band_y = int(h * float(self.get_parameter('front_band_ratio').value))
        ...
        #     band_c = cv2.morphologyEx(band_c, cv2.MORPH_CLOSE, k2, iterations=1)
        #     band_c = cv2.morphologyEx(band_c, cv2.MORPH_OPEN,  k2, iterations=1)
        #     front_fill = float(band_c.mean() / 255.0)
        
        # obs: obstacle mask , steer_norm: steering direction, front_fill: how blocked the front is
        return obs, float(steer_norm), float(front_fill)
        
        

    async def execute_callback(self, goal_handle):
        goal = goal_handle.request
        # vision_rate = rate(self.get_clock(), 20.0) # Hz
        # period_s = 1.0 / 20.0
        ...
        while rclpy.ok():
            ...
            if pose is None or self.last_image is None:
                self.get_logger().info("Waiting for odom/image")
                # await vision_rate.sleep()
            ...
            speed_scale = max(0.0, 1.0 - 1.5*front_fill) * max(0.2, math.cos(abs(ang))) 
            v = vmax * speed_scale
            # if v > 0.0:
